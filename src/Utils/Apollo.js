import { ApolloClient, InMemoryCache, gql } from '@apollo/client';
/*
    @uri uri
    @entity entity
    @columns columns
    @page page
    @pageSize pageSize
    @columnsToFilter Array with columns name to filter
    @value String with value to filter on every column selected
    @defaultFilter Array with filters by default
*/
export const ClientGraphQL = (uri, entity, columns, page, pageSize, columnsToFilter, value, defaultFilter) => {
  return new Promise((resolve, reject) => {
    const client = new ApolloClient({
      cache: new InMemoryCache({
        addTypename: false
      }),
      uri: uri,
      headers: {
        authorization: localStorage.getItem('id_token') || 'Bearer eyJ4NXQiOiJaalJtWVRNd05USmpPV1U1TW1Jek1qZ3pOREkzWTJJeU1tSXlZMkV6TWpkaFpqVmlNamMwWmciLCJraWQiOiJaalJtWVRNd05USmpPV1U1TW1Jek1qZ3pOREkzWTJJeU1tSXlZMkV6TWpkaFpqVmlNamMwWmdfUlMyNTYiLCJhbGciOiJSUzI1NiJ9..gy7fE1Wf8GFQix9KnmCFI-3IIIYn1Wiq6GeIAca7gZnn5SCH3OeNpRYdVIih3Xs0t0EJkW4YuD5LTIICMNywpVKh93FfoYVIhbw1ghDcnBmcv__VwKlIuX7CTOLjzpID3PqYfIzkEeaxdaVx5zFutcWcKzzBGpUR8FypTYvCMnpWa9RZKhUvGvhAG_KC4HsnHVDkaE1HVmuR1_fOsCJg8E2JUxVFKnBe8uF40m_wyT_MeKRQvCF-2OKqVmsbF9Qi9hH-Juf7_X2WiKfbcAj-5p85KDxocUEGK04coVU2kIb0-886G5My-4IiqqpjxDGgprg4GescWkXxYaBTootCbQ',
      },
    })
    const QUERY = gql`
          {
            find${entity}List(
              page:{ number: ${page} size: ${pageSize}}
              
              ${columnsToFilter || defaultFilter ?
        `filters:[
                ${defaultFilter ? defaultFilter.map(filter => { return `{mod: ${filter.mod} col: "${filter.col}" val: "${filter.val}"}` }) : ''}
                ${columnsToFilter.map(column => { return `{mod: LK col: "${column}" val: "${value}"}` })}
              ]`
        : ''}
              ){
              totalElements
              totalPages
              content{
                ${columns.map(item => {
          return item.accessor
        })}
              }
            }
          }
          `
    client.query({
      query: QUERY
    }).then(result => {
      let payload = {
        columnsToFilter: columnsToFilter,
        filterValue: value,
        data: result.data[`find${entity}List`].content,
        pages: result.data[`find${entity}List`].totalPages,
        pageSize: pageSize,
        page: page,
      }
      resolve(payload)
    }).catch(error => {
      reject(error)
    })
  })
}
